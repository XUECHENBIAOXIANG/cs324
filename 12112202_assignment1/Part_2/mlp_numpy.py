...
class MLP(object):
    def __init__(self, n_inputs, n_hidden, n_classes,learningrate=1e-2):
        """
        ...
        self.layers = []
        self.layers.append(Linear(n_inputs, n_hidden[0],self.learningrate))
        self.layers.append(ReLU())
        for i in range(1,len(n_hidden)):
            self.layers.append(Linear(n_hidden[i-1], n_hidden[i],self.learningrate))
            self.layers.append(ReLU())
        self.layers.append(Linear(n_hidden[-1], n_classes,self.learningrate))
